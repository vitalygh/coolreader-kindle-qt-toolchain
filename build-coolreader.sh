...
targetpath="target.path = $binpath"
...
if ! grep -q "$targetpath" $libpath/cr3-kindle/src/src.pro; then
	...
	sed -i "s*target.path = /mnt/us/cr3*$targetpath*" $libpath/cr3-kindle/src/src.pro
